...
let visibililyVal=document.getElementById('visibililyval');
...
function getWeatherdetails(name, lat, lon, country, state) {
    ...
    let AIR_POLLUITION_API_URL=`http://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${api_key}`;
    ...
    fetch(WEATHER_API_URL).then((res) => res.json()).then((data) => {
        ...
        currentWeatherCard.innerHTML = `
             <div class="current-weather">
                        <div class="details">
                            <p>Now</p>
                            <h2>${(data.main.temp - 273.15).toFixed(2)}&deg;C</h1>
    ...
    //    let sRiseTime=moment.utc(sunrise,'x').add(timezone, 'seconds').format('hh:mm A');
            let sRiseTime = moment.unix(sunrise).utc().add(timezone, 'seconds').format('hh:mm A');
let sSetTime  = moment.unix(sunset).utc().add(timezone, 'seconds').format('hh:mm A');
    //     let sSetTime=moment.utc(sunset,'x').add(timezone, 'seconds').format('hh:mm A') ;
        ...
        visibililyVal.innerHTML=`${visibility/1000}km`
    ...
    fetch(FORECAST_API_URL).then((res) => res.json()).then((data) => {
     ...
     fetch(AIR_POLLUITION_API_URL).then((res) => res.json()).then((data) => {
        let {co,no,no2,o3,so2,pm2_5,pm10,nh3}=data.list[0].components;
         aqiCard.innerHTML=`
            <div class="card-head">
                        ...
                        <div class="air-indices"><i class="fa-regular fa-wind fa-3x"></i>
                            <div class="item">
                                <p>PM2.5</p>
                                <h2>${pm2_5}</h2>
                            </div>
